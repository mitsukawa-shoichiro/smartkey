...
def accesslogs(page: ft.Page):
    ...
    start_text = ft.TextField(label="開始日時", width=200,
                              height=48, max_length=50)
    end_text = ft.TextField(label="終了日時", width=200, height=48, max_length=50)
    ...
    # 検索フィールドの定義
    searchcardname = ft.TextField(
        label="カード名", width=200, height=48, max_length=50)
    searchmethod = ft.DropdownM2(
        label="認証   方式", value=None, width=85, height=45)
    ...
    start_time = ft.TimePicker(value=time(
        0, 0), on_change=lambda e: update_start_textbox())
    end_time = ft.TimePicker(value=time(
        23, 59), on_change=lambda e: update_end_textbox())

    searcheventtype = ft.DropdownM2(
        label="入室/  退室", value=None, width=85, height=45)
    ...
    def search_logs(e):
        ...
        search_eventtype = int(searcheventtype.value.strip()
                               ) if searcheventtype.value else None
        start_dt = None
        end_dt = None
        ...
        if start_text.value != "" and end_text.value != "":
            ...
            if start_dt and end_dt and end_dt < start_dt:
                ...
                dialog.actions = [
                    ft.TextButton(
                        "閉じる", on_click=lambda e: page.close(dialog)),
                ]
                page.open(dialog)
                return
        ...
        # ③ 件数取得→総ページ
        cnt = db.count_filtered_logs(
            search_cardname, search_method, search_eventtype, start_dt, end_dt)
    ...
    pagination_controls = ft.Row(
        [prev_btn, page_label, next_btn], alignment=ft.MainAxisAlignment.CENTER)
    ...
    startdate_btn = ft.ElevatedButton(text="開始日時を選択",
                                      ...
                                      on_click=lambda e: open_datepicker(
                                          start_date),
                                      style=ft.ButtonStyle(
    ...
    sort_btn = ft.IconButton(
        icon=ft.Icons.ARROW_CIRCLE_DOWN, on_click=togle_sort,)
    eventtype_row = ft.Row(
        [ft.Text("入退室の日時", weight="bold", size=14), sort_btn])
    # テーブル定義
    table = ft.DataTable(
        columns=[
        ...
        content=ft.Column(
            [
                ft.Row([searchcardname, searchmethod,
                        ft.Container(searcheventtype, margin=ft.margin.only(
                            right=56)), reset_btn
                        ]),
                ft.Row([startdate_btn,
                        ft.Container(start_text, margin=ft.margin.only(
                            right=98)), search_btn
                        ]),
                ft.Row([enddate_btn,
                        ft.Container(end_text, margin=ft.margin.only(
                            right=98)), show_all_btn
                        ])
            ],
            spacing=5,
