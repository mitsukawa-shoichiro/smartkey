...
class SsmBleClient:
    ...
    def __init__(self, config):
        ...
        self.isLogin = False
        SsmBleClient._last_instance = self
    ...
    async def get_state(self):
        self.__isWait = True
        tag = 'SampleUnlock'.encode()
        command = bytes([ITEM_CODE.MECH_STATUS.value, len(tag)]) + tag
        print(f"cmd send unlock data: [{command}] encrypt: {True}")
        await self._send(command, True)
        await self._wait()
    ...
    def __handleNotification(self, handle, data):
        ...
        if self._op_code == OP_CODE.RESPONSE.value:
            if self._item_code == ITEM_CODE.LOGIN.value:
                ...
                print(f"history!")
            elif self._item_code == ITEM_CODE.MECH_STATUS.value:
                byte = data[2:]
                result = struct.unpack("<HhhB", byte)
                print(result)
                battery = result[0] * 2 / 60
                if battery > 100:
                    battery = 100
                print(f"電池残量：{battery}%")
